...
| `--title` | 新闻主标题 | ✅ |
| `--subtitle` | 副标题/关键信息 | ❌ |
| `--tag` | 左上角标签（不填则自动判断：财经/政治/科技/体育/悼念/速递） | ❌ |
| `--bullets` | 要点列表（第2页展示） | ❌ |
| `--context` | 背景信息（第3页展示） | ❌ |
| `--data` | 关键数据，格式：`标签:数值`（第4页卡片展示） | ❌ |
| `--timeline` | 时间线事件（第5页展示） | ❌ |
| `--quotes` | 各方引用，格式：`内容:来源`（第6页展示） | ❌ |
| `--highlight` | 核心观点/金句（第7页大字展示） | ❌ |
| `--ending` | 结尾文字（第8页） | ❌ |
